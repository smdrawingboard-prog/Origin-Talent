...
- `Origin-Talent-Client-Service-Agreement.pdf` — Origin Talent's actual
  placement-terms contract (appointment, services provided, client
  responsibilities, fees, TES, permanent placements, 90-day replacement
  guarantee, confidentiality, POPIA, non-solicitation, liability, health &
  safety, dispute resolution, termination). Business details are prefilled;
  client details and signatures are left blank to complete per engagement.
...
the source content changes, update the data in that script and re-run it
instead of editing the PDFs directly.

**Source of truth:** the two form PDFs and the agreement follow Origin
Talent's own source documents (`Client_Staffing_Enquiry_Form__ONLINE_FORM_OT.docx`,
`Candidate_Online_Application_Form_OT.docx`, `CLIENT_SERVICE_AGREEMENT__ORIGIN_TALENT.docx`),
not the live `index.html` forms — where they disagree, the .docx wins in
the PDFs. Known differences from the live online forms / Supabase schema,
not yet applied to the site:

- **Client enquiry form** — the PDF has a "Candidate nationality" field
  (South African / Zimbabwean / Lesotho / Other) and a fixed language
  checklist (English/Zulu/Afrikaans/Xhosa/Sotho/Tswana/Other) that the
  online form and `client_enquiries` table don't have.
- **Candidate application form** — the PDF adds Gender and a "convicted of
  a criminal offence?" question, and a "Documents to attach" checklist
  (photo, CV, ID/passport, reference letter, bank statement, licence,
  certificates, police clearance) that the online form has no equivalent
  for — `candidate_applications` has no file-storage columns and the site
  currently tells applicants *not* to send banking documents through an
  unsecured channel. The PDF checklist is a physical "bring these copies"
  list, not a live upload feature.

If you want the online form and database to match the PDFs exactly (new
columns + RLS updates in a `004_*.sql` migration, matching `index.html`
fields, and a decision on whether/how to collect uploads — especially the
bank statement — securely), that's a follow-up, not yet done here.
